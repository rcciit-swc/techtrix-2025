...
export const generateReferralCode = async (): Promise<string | null> => {
    ...
    const { count } = await supabaseServer.from("transactions").select("*").eq("referrer", userId);
    const referralCount = count ?? 0;
    if (referralCount >= maxReferrals) return null;

    const { data } = await supabaseServer.from("referral_codes").select("*").eq("community_name", "RCCIIT").single(); // This should be dynamic
...
function decryption(encryptedPayload: string): { userId: string, referralCode: string } {
