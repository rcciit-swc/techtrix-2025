...
import { TechtrixCategories } from '@/utils/constraints/constants';
...
const EventDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!eventId) return;
      const data = (await getEventByID(eventId)) as EventData | null;
      setEventData(data);
  ...
  if (!eventData) {
    return (
      <div className="text-white text-center mt-20 min-h-screen">
        {' '}
        <video
      ...
      <div className="relative max-w-5xl mx-auto p-4 sm:p-6 pt-24 sm:pt-48 mt-10 mix-blend-normal">
        ...
        >
              ...
              <div className="relative flex flex-col justify-start w-full sm:w-2/3 px-4 sm:px-6 py-6 sm:py-8 rounded-3xl bg-gradient-to-tl from-black via-black/70 to-gray-800 border border-white shadow-lg">
                ...
                {eventData?.coordinators.map((coordinator, index) => (
                  <p key={index} className="font-alexandria text-gray-300 mt-2">
                    {coordinator.name}:{' '}
                    <Link
                      className="text-yellow-200 hover:text-green-200"
          ...
          <div className="mt-4 sm:mt-6 text-white text-base sm:text-lg">
            ...
            {eventData?.schedule && (
              <div dangerouslySetInnerHTML={{ __html: eventData.schedule }} />
            )}
          </div>
          <div className="mt-4 sm:mt-6 text-white text-base sm:text-lg">
            <h1
